...
$hasil_guru = mysqli_query($conn, $sql);
?>
...
.icon-action:hover {
  ...
  <body>
    ...
    <div class="d-flex justify-content-between mb-2">
        ...
        <div>
            <a href="addguru-admin.php" class="btn btn-success">Tambah Guru</a>
        </div>
    </div>
    
    <div class="mb-3 d-flex justify-content-end">
        <input type="search" class="form-control" style="width: 200px;" id="searchInput" onkeyup="searchTable()" placeholder="Cari Guru...">
    </div>
    ...
    <thead>
        <tr>
            ...
            <th onclick="sortTable(1)">NIP <span class="iconify" data-icon="akar-icons:arrow-up-down"></span></th>
            <th onclick="sortTable(2)">Nama Lengkap <span class="iconify" data-icon="akar-icons:arrow-up-down"></span></th>
            <th onclick="sortTable(3)">Jenis Kelamin <span class="iconify" data-icon="akar-icons:arrow-up-down"></span></th>
            <th onclick="sortTable(4)">No Telepon <span class="iconify" data-icon="akar-icons:arrow-up-down"></span></th>
            <th>Action</th>
        </tr>
    </thead>
    <tbody>
    ...
    foreach($hasil_guru as $guru): ?>
        <tr>
            <td><?php echo $no++; ?></td> <!-- Gunakan dan increment variabel counter -->
            ...
            <td>
        <!-- Tombol View -->
        <a href="viewguru-admin.php?nip=<?php echo $guru['nip']; ?>" class="btn btn-success btn-sm">
            <iconify-icon icon="akar-icons:eye"></iconify-icon>
        </a>
        <!-- Tombol Edit -->
        <a href="editguru-admin.php?nip=<?php echo $guru['nip']; ?>" class="btn btn-primary btn-sm">
            <iconify-icon icon="bxs:edit"></iconify-icon>
        </a>
        <!-- Tombol Delete -->
        <button onclick="confirmDelete('<?php echo $guru['nip']; ?>')" class="btn btn-danger btn-sm">
...
function sortTable(n) {
  ...
  while (switching) {
    ...
    for (i = 1; i < (rows.length - 1); i++) {
      ...
      // Cek apakah ini kolom nomor (kolom ke-0)
      if (n === 0) {
        // Mengubah string menjadi integer untuk perbandingan numerik
        ...
        // Untuk kolom selain nomor, lakukan pengurutan string
        if (dir == "asc") {
...
function confirmDelete(nip) {
    var confirmDelete = confirm("Apakah Anda yakin ingin menghapus guru dengan NIP: " + nip + "? (SEMUA DATA YANG TERKAIT GURU INI AKAN TERHAPUS)");
    if (confirmDelete) {
        window.location.href = 'deleteguru-admin.php?nip=' + nip;
    }
}
</script>
