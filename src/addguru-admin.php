...
if (isset($_POST['submit'])) {
    ...
    // Pengecekan NIP di database
    $checkNipQuery = "SELECT nip FROM guru WHERE nip = ?";
    $stmt = mysqli_prepare($conn, $checkNipQuery);
    mysqli_stmt_bind_param($stmt, 's', $nip);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);

    if (mysqli_num_rows($result) > 0) {
        // Jika NIP sudah ada di database
        echo "<script>alert('Gagal: NIP sudah terdaftar di sistem');</script>";
    } else {
        // Jika NIP belum ada, lanjutkan dengan penambahan data guru
        $query = "INSERT INTO guru (nip, nama, tempat_lahir, tanggal_lahir, jenis_kelamin, agama, no_hp, email, alamat, password, foto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, 'sssssssssss', $nip, $nama, $tempat_lahir, $tanggal_lahir, $jenis_kelamin, $agama, $no_hp, $email, $alamat, $password, $filename);
        
        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Data berhasil ditambahkan!'); window.location='daftarguru-admin.php';</script>";
        } else {
